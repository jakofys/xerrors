...
func TestAsInterface(t *testing.T) {
	...
	cases := []struct {
		...
		},
		{
			...
			input:       stdErrors.Join(stdErrors.New("error"), errImplement{msg: implementMsg}),
			expectedNil: true,
		},
	}
	for _, c := range cases {
		t.Run(c.name, func(t *testing.T) {
			result := errors.AsInterface[Implementer](c.input)
			if c.expectedNil {
...
func TestAsInterfacePanicOnNoneInterface(t *testing.T) {
	assert.Panics(t, func() {
		e := errors.AsInterface[errUnimplement](errUnimplement{})
		assert.Nil(t, e)
	})
	assert.Panics(t, func() {
		e := errors.AsInterface[*errUnimplement](errUnimplement{})
